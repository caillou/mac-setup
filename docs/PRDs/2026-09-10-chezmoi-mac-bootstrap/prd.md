...
- Auto-brightness is stored in root's CoreBrightness preferences as one `AutoBrightnessEnable` flag per display entry (confirmed on the personal Mac, where it is false). The display script runs on every apply: `sudo -H defaults export` of that domain, set the flag to false on every display entry, `sudo -H defaults import` only if something changed, and only then restart the brightness daemon; read back; mark success or print the manual instruction. Never edit the plist file directly. Display identifiers differ per machine, so the script iterates the entries. Reading needs sudo, so one prompt per apply is accepted. On a Mac where every entry is already false it is a true no-op with no daemon restart.
...
- One issue: rename the GitHub repo; move the local checkout to `~/repos/caillou/dotfiles`; `chezmoi init --source ~/repos/caillou/dotfiles` so chezmoi knows where the repo is; copy the three Hammerspoon files and the Karabiner source into the repo; switch the iTerm2 non-ASCII font to the cask and export the prefs into the repo; back up every target listed by `chezmoi managed` plus fish's universal variables file with tar, since chezmoi keeps no backups; purge dead universal variables; remove `~/.nvmrc`; then apply in two steps: `chezmoi apply --exclude scripts`, open a new fish and check prompt, abbreviations and `node --version`, then `chezmoi apply` for the scripts; `asdf reshim`; archive `caillou/keyboard` and `caillou/karabiner.ts` on GitHub with a README pointer and remove their local checkouts. `chezmoi diff` previews files only, not script effects, which is why the apply is split. The repo files written by the earlier issues are the source of truth; nothing is adopted from the home directory.
