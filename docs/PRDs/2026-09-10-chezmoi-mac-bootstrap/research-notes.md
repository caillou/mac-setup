...
- Homebrew's asdf is 0.20.x (same Go lineage as 0.16). This Mac has no `fish_plugins`: `~/.config/fish/` holds the old `fishfile` and fisher 3.2.9 at `functions/fisher.fish`, which has no `update` subcommand, so the migration retires fisher 3 before the first apply. Pure's `_pure_set_default` calls live in `conf.d/pure.fish` (73 of them).
...
- **Core, fonts (all Macs, managed included):** font-ibm-plex-mono, font-ibm-plex-sans, font-anonymice-nerd-font (iTerm2's non-ASCII font for prompt glyphs). Today the profile references `AnonymiceNerdFontCompleteM- 16`, a Nerd Fonts v2 file dropped into `~/Library/Fonts` in 2023 and present on no other Mac. The cask (3.5.1) installs v3 files `AnonymiceProNerdFontMono-*.ttf`, family "AnonymicePro Nerd Font Mono"; the profile's non-ASCII font becomes `AnonymiceProNerdFontMono-Regular 16` and the old v2 file is deleted, so both Macs render iTerm2 from the same profile with the same Homebrew fonts. Blex Nerd Font is unused and dropped.
- **Core, casks:** iterm2, visual-studio-code, google-chrome, firefox, hammerspoon, karabiner-elements, alfred, istat-menus, homerow, the-unarchiver, docker-desktop, obsidian, slack, microsoft-teams, ngrok, vlc, handbrake-app (Homebrew's current name for the HandBrake cask).
